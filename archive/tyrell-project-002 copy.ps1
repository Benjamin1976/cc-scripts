...
$vmUsr = "benjamin"
$vmPass = "benjamin_123"
$subscription = "60980e3a-5f38-4652-8ff3-f7a7741e315e"

$grp = "az305-project-002"
$ver = "002"
...
# if ($createTrafficMgr) {
    ...
    foreach ($type in $types) {
        ...
        if ($createTrafficMgr) {
            $sp = az network traffic-manager profile create `
                        --name $tfmgrName `
                        --resource-group $grp `
...
if ($createAppGW) {
    ...
    foreach ($bepool in $backendPools) {
        $cmdout = az network application-gateway address-pool create -g $grp --gateway-name $appGWName --name $bepool[0] --servers $bepool[1]
        $sp += $cmdout
    }
    ...
    foreach ($pathmap in $pathmaps) {
        $cmdout = az network application-gateway url-path-map create --resource-group $grp --gateway-name $appGWName --name $appPathMapName `
        --paths $path1 --address-pool $pathmap[0] --http-settings  $pathmap[1] `
        --default-address-pool $pathmap[0] --default-http-settings  $pathmap[1]
        $sp += $cmdout
    }
    ...
    $sp = az network application-gateway rule update --resource-group $grp --gateway-name $appGWName --name $gwRule --priority 99 --http-listener $appGWListener `
                 --rule-type PathBasedRouting --address-pool  $backendPools[$defaultBePool] --http-settings $defaultHttpSettings1 --url-path-map $appPathMapName
