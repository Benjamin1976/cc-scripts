...
def execLogCmd(cmd):
    writeFile(file, cmd)
    cmdOut = subprocess.check_output(cmd.split(" "))     
...
cmd = "az network public-ip show --name {publicIpName --resource-group {grp --query '{address: ipAddress}' --output json" 
pip = json.loads(cmd)
print(pip)
# {pip = {cmdout.address
...
execLogCmd("az group create --location eastus --name" + grp)
...
for sub in subs:
    for stype in types:
        ...
        location = sub[0]
        locCode = sub[1]
        vnetName = ["tyrell", stype, "vnet", locCode, ver].join("-")
        vnetPrefix = "10." + sub[2] + ".0.0/16"
        subnetName = ["tyrell", stype, "subnet", locCode, ver].join("-")
        ...
        sp_plan = [sppname, stype, locCode, ver].join("-")
        app_name = [appname, stype, locCode, ver].join("-")
...
trafficmgrs = []
for stype in types:
    tfmgrName = [tfmgrPre, stype, ver].join("-")
    ...
    for sub in subs:
        locCode = sub[1]
        endpointName = [appname, stype, locCode, ver].join("-")
        endpointName = [appname, stype, locCode, ver].join("-")
