...
$vmUsr = "benjamin"
$vmPass = "benjamin_123"
$subscription = "c95c0e44-d963-4aa7-8ee5-b8bd005e19f7"
...
foreach ($sub in  $subs) {
    foreach ($type in  $types) {
        ...
        if ($createAppsService) {
            ExecLogCmd -cmd "az webapp create --resource-group $grp --name $app_name --plan $sp_plan --vnet $vnetName `
...
foreach ($type in $types) {
    ...
    if ($createTrafficMgr) {
        ExecLogCmd -cmd "az network traffic-manager profile create `
                    --name $tfmgrName `
                    --resource-group $grp `
    ...
    foreach ($sub in $subs) {
        ...
        if ($createTrafficMgr) {
            ExecLogCmd -cmd "az network traffic-manager endpoint create --name $endpointName `
...
if ($createAppGW) {
    ...
    ExecLogCmd -cmd "az network vnet create -g $grp --location $location -n $vnetName --address-prefix $vnetPrefix `
    ...
    ExecLogCmd -cmd "az network application-gateway create -g $grp -n $appGWName --vnet-name $vnetName --subnet $subnetName `
    ...
    foreach ($bepool in $backendPools) {
        ...
        ExecLogCmd -cmd "az network application-gateway address-pool create -g $grp --gateway-name $appGWName `
    ...
    foreach ($pathmap in $pathmaps) {
        ExecLogCmd -cmd "$az network application-gateway url-path-map create --resource-group $grp --gateway-name $appGWName `
    ...
    ExecLogCmd -cmd "az network application-gateway rule update --resource-group $grp --gateway-name $appGWName --name $gwRule `
    ...
    ExecLogCmd -cmd "az network application-gateway http-settings update --gateway-name $appGWName `
    ...
    ExecLogCmd -cmd "az network application-gateway probe create --resource-group $grp --gateway-name $appGWName --name $healthProbeName `
                                                --from-settings true `
                                                --port 80 `
